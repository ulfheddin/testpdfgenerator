...
<head>
	...
	<script>		
		 ...
		 function myAddRow() {
			...
			cell1.innerHTML = '<select style="text-align:center; width:100%;" name="type" id="test" class="form-control" onchange="this.nextElementSibling.value=this.value" required><option name="register" value="REGISTER">REGISTER</option><option name="renew" value="RENEW">RENEW</option><option name="redemption" value="REDEMPTION">REDEMPTION</option><option name="transfer" value="TRANSFER">TRANSFER</option><option name="purchase" value="PURCHASE">PURCHASE</option></select>'; 
			...
			inp2.name += len;
			inp2.value = '';
			cell3.innerHTML = '<input name="quantity" id="quantity" style="text-align:center; width:100%;" class="form-control" style="text-align:center;" value="1" size="4" onblur="add_subtotal(this); Icann(this);" required>';
			var inp3 = cell3.getElementsByTagName('input')[0];
			inp3.name += len;
			inp3.id += len;
			inp3.value = '1';
			cell4.innerHTML = '<select style="text-align:center; width:100%;" name="duration" id="duration" class="form-control" onClick="Icann(this); add_subtotal(this);" required><option selected="selected" name="dur1" value="1 year">1 year</option><option name="dur2" value="2 years">2 years</option><option name="dur3" value="3 years">3 years</option><option name="dur4" value="4 years">4 years</option><option name="dur5" value="5 years">5 years</option><option name="dur6" value="6 years">6 years</option><option name="dur7" value="7 years">7 years</option><option name="dur8" value="8 years">8 years</option><option name="dur9" value="9 years">9 years</option><option name="dur10" value="10 years">10 years</option></select>';
			...
			inp52.htmlFor += len;
			cell6.innerHTML = '<input style="text-align:center; width:100%;" type="text" size="4" name="subtotal" id="subtotal" placeholder="X.XX" class="form-control" required><br/><br/><input style="text-align:center; width:100%;" type="text" size="4" name="fee" id="fee" placeholder="X.XX" class="form-control" required>';
			var inp6 = cell6.getElementsByTagName('input')[0];
			inp6.name += len;
			inp6.id += len;
			inp6.value = '';
			var inp61 = cell6.getElementsByTagName('input')[1];
			inp61.name += len;
			inp61.id += len;
			inp61.value = '0.00';
		...
		function add_total(){
			...
			document.getElementById('subtotalfinal').value = total_cost.toFixed(2);	
			document.getElementById('initial_charge').value = total_cost.toFixed(2);
...
<body>
		<form name="order_receipt" method="post" class="form-inline" target="_blank">
				...
				<div class="col-sm-5">		
					<table class="upper">
						<tr>
							...
							<td><input class="form-control" type="text" name="order_date" id="order_date" placeholder="mm/dd/yyyy" title="The date when transaction happened in format: mm/dd/yyyy" required></td>
						</tr>
						<tr>
							<td><b>Order Number </b></td>
							<td><input class="form-control" type="text" name="order_number" placeholder="NC order ID" title="Namecheap Order ID" required></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td>
								<select name="payment_source" id="payment_source" class="form-control" required> 	
									...
									<option name="Funds" value="Funds">Funds</option>
					    			</select>
					    		</td>
				    		</tr>
				    		<tr>
						<td><b>Initial Charge </b></td>
						<td><input class="form-control" type="text" name="initial_charge" id="initial_charge" placeholder="X.XX" pattern="^\d+([.]\d{2})?$" title="Total amount of the order. Format X.XX without $ sign, decimals separated by ." required></td>
						</tr>
						<tr>
							<td><b>Final Cost </b></td>
							<td><input class="form-control" type="text" name="final_cost" id="final_cost" placeholder="X.XX" pattern="^\d+([.]\d{2})?$" title="Total amount of the order minus refunded amount (if refunded). Format X.XX without $ sign, decimals separated by ." pattern="^\d+([.]\d{2})?$" required></td>
						</tr>
						<tr>
		...
		<div class="row">	
			<div id="productsdiv" style="width:80%;" align="center">
				 <table id="purchaseItems" name="purchaseItems" class="table table-striped" border="0">
					<tr>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Type</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Product</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Qty</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Duration</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Price</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">Subtotal</th>
					        <th style="text-align:center; width:;" bgcolor="#d9d9d9">&nbsp;</th>
					        <th style="text-align:center; width:" bgcolor="#d9d9d9">&nbsp;</th>
					</tr>
					<tr>
						<td>
						        <select name="type1" class="form-control" style="text-align:center; width:100%;" required> 	
					        ...
					        <td>
							<input name="quantity1" id="quantity1" class="form-control" style="text-align:center; width:100%;" size="4" value="1" onblur="add_subtotal(this); Icann(this);" required>
					        </td>
					        <td>
					        ...
					        <td>
						        ...
						        <p text-align="left" style="width:100%;"><b>ICANN Fee:</b></p>
					        ...
					        <td>
						        ...
						        <input style="text-align:center; width:100%;" value="0.00" type="text" size="4" name="fee1" id="fee1" class="form-control" required>
					        </td>
					        <td>
				...
				<div style='text-align:right; margin-right:10%;'>
					<p><b>Subtotal &nbsp;</b><input style="text-align:center; font-weight:bold;" type="text" size="4" class="form-control" name="subtotalfinal" id="subtotalfinal" value="0.00" placeholder="X.XX" required></p>
					<p><b>Total &nbsp;</b><input style="text-align:center; font-weight:bold;" type="text" size="4" class="form-control" name="total" id="total" value="0.00" placeholder="X.XX" onblur="add_total()" required></p>
				</div>
			</div>
		</div>
			...
			<div style='margin-left:35%;'>						
				<input type="hidden" name="form_id" value="order_receipt"><br/>
